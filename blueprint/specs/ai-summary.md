...
- worker text extraction and AI-powered summary generation
...
3. The API stores the PDF locally (one file per workshop).
4. The API updates `workshop.pdfUrl` and clears any existing `workshop.aiSummary`.
5. The API enqueues a job to the `ai-summary-queue` with payload `{ workshopId, filePath }`.
...
5. The worker generates a concise summary from the cleaned text using an AI language model.
6. The worker updates `workshop.aiSummary` with the generated summary.
7. The worker invalidates the cached workshop detail in Redis.
...
- The worker generates summaries using an AI language model. The summary should cover key topics, target audience, and speaker information where available.
...
- The worker generates an AI-powered `aiSummary` from the extracted and cleaned PDF text.
- The worker invalidates the cached workshop detail in Redis.
