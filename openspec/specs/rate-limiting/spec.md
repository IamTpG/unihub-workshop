...
The system SHALL enforce a stricter rate limit of 5 requests per 60-second window per IP on auth endpoints (e.g., login, request-otp) to prevent spam.

#### Scenario: Auth request spam
- **WHEN** a client sends 6 auth requests within 60 seconds
...
The system SHALL enforce a rate limit of 10 requests per 60-second window per authenticated user on workshop registration endpoints.
...
The system SHALL apply higher rate limits to STAFF and ADMIN roles (e.g., 200 requests per 60 seconds) compared to STUDENT role (e.g., 100 requests per 60 seconds).
